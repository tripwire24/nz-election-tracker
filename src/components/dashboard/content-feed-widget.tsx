...
const sourceColour: Record<string, string> = {
  ...
  media: "bg-zinc-700/40 text-zinc-300 ring-1 ring-zinc-600/30",
    ...
    return (
      <DashboardCard title="Breaking Content" badge="Feed" accent="#f97316">
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="rounded-lg border border-zinc-700/30 px-3 py-2.5">
              <div className="h-4 w-3/4 rounded bg-zinc-700/30 animate-shimmer" />
              <div className="mt-2 flex gap-2">
                <div className="h-3 w-16 rounded bg-zinc-700/30 animate-shimmer" />
                <div className="h-3 w-10 rounded bg-zinc-700/30 animate-shimmer" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="max-h-[360px] overflow-y-auto space-y-3 pr-1 scrollbar-thin">
        {items.map((item) => (
          <a
            ...
            className="flex items-start gap-3 rounded-lg border border-zinc-700/30 px-3 py-2.5 transition-colors hover:border-zinc-700 hover:bg-zinc-800/30"
          >
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${sourceColour[item.source_type] ?? "bg-zinc-700/40 text-zinc-400 ring-1 ring-zinc-600/30"}`}>
                  {item.source_name}
                </span>
