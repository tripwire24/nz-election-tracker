  ...
  return (
    <DashboardCard title="Seat Projection" badge="Sainte-Laguë" accent="#6366f1">
      ...
      <div className="relative mt-1 h-5">
        <div
          className="absolute top-0 h-5 border-l-2 border-dashed border-zinc-500"
          style={{ left: `${(MAJORITY / TOTAL_SEATS) * 100}%` }}
        />
        <span
          className="absolute top-0.5 text-[9px] font-medium text-zinc-500"
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
        {seats.map((p) => (
          <div key={p.short} className="flex items-center gap-1.5 text-xs text-zinc-400">
      ...
      <div className="mt-3 flex gap-4 text-xs">
        ...
        <span className="text-zinc-600 py-1">{totalAllocated} seats allocated</span>
      </div>
    </DashboardCard>
  );
}
