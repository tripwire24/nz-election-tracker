...
export function PagePill({
  ...
  return (
    <span
      className={cn(
        "rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-xs md:px-3 md:py-1.5 md:text-sm text-neutral-300",
        className,
      )}
    >
      {children}
...
export function PageHero({
  ...
  return (
    <section
      className={cn(
        "rounded-[1.25rem] md:rounded-[1.75rem] border border-white/10 bg-[linear-gradient(135deg,rgba(28,28,28,0.98),rgba(18,18,18,0.98))] p-4 shadow-[0_20px_50px_rgba(0,0,0,0.32)] md:p-6 lg:p-8",
        className,
      )}
    >
      <div
      ...
      >
        <div className="space-y-3 md:space-y-4">
          ...
          <div className="space-y-2 md:space-y-3">
            <h1 className="text-2xl font-semibold tracking-tight text-neutral-100 md:text-3xl lg:text-4xl">
              {title}
            </h1>
          ...
          {pills.length > 0 && <div className="flex flex-wrap gap-2">{pills}</div>}
        </div>

        {aside ? (
          <div className="hidden lg:block h-fit self-start rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-5 ring-1 ring-white/5">
            {aside}
          </div>
        ) : null}
      </div>
